...
--오직 한 행에 한 컬럼만 담고 있는 dumy 테이블 이다.
...
--substr(문자열 자르기),concat(문자열 연결) 1부터 시작
SELECT
    'abcdef' AS ex,
    SUBSTR('abcdef',1,4),
    CONCAT('abc','def') --CONCAT은 매개값 2개만 가능
FROM dual;
...
--replace()
SELECT
    REPLACE('My dream is a president','president','programmer') --매개값이 총 3개
FROM dual;

SELECT
    REPLACE('My dream is a president',' ','') --매개값이 총 3개
FROM dual;
