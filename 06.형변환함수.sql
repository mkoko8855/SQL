...
SELECT TO_CHAR(sysdate, 'YYYY-MM-DD DAY PM HH:MI:SS') FROM dual; --자바에서는 MM으로 분을 표현, SQL차이
SELECT TO_CHAR(sysdate, 'YY-MM-DD HH24:MI:ss') FROM dual; --자바에서는 MM으로 분을 표현, SQL차이

--사용하고 싶은 문자를 ""로 묶어서 전달한다.
...
SELECT TO_CHAR(salary, '$99,999') AS salary --L을 이용하면 원을 표시할 수 있다.
FROM employees;

--문자를 숫자로 TO_NUMBER(값, 형식)
SELECT '2000' + 2000 FROM dual; --암묵적 변환.자동 형변환(문자 -> 숫자)
SELECT TO_NUMBER('2000') + 2000 FROM dual; --명시적 형 변환
SELECT '$3,300' + 2000 FROM dual; -- 변환 불가
SELECT TO_NUMBER('$3,300','$9,999') + 2000 FROM dual;

--문자를 날짜로 변환하는 함수 TO_DATE(값, 형식)
SELECT TO_DATE('2023-04-13') FROM dual;
SELECT sysdate - '2021-03-26' FROM dual; --연산 불가
SELECT sysdate - TO_DATE('2021-03-26') FROM dual; -- 일자로 표현
SELECT TO_DATE('2021/12/25','YY-MM-DD') FROM dual;
--주어진 문자열을 모두 변환해야 한다.
SELECT TO_DATE('2021-12-25 12:23:50','YY-MM-DD') FROM dual; --변환 불가
SELECT TO_DATE('2021-12-25 12:23:50','YY-MM-DD HH:MI:SS') FROM dual;
...
SELECT
    job_id,
    salary,
    DECODE(job_id,'IT_PROG',salary*1.1,'FI_MGR',salary*1.2,'AD_VP',salary*1.3) AS result
FROM employees;
...
SELECT first_name,manager_id,job_id,
       DECODE(manager_id,100,'사원',120,'주임',121,'대리',122,'과장','임원') AS result
FROM employees
