...
--두 테이블 모두 가지고 있는 컬럼의 경우 반드시 명시해 줘야 된다.
...
AND loc.location_id=d.location_id --2
AND loc.state_province='California'; --1 WHERE절 해석 순서
...
--일반 조건에도 (+)를 붙이지 않으면 외부 조인이 풀리는 현상이 발생.
...
AND j.department_id(+) = 80; --일반조건에서 outer 조인을 유지하려면 (+)
